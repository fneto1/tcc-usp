...
def get_system_metrics(base_url):
    """Coleta metricas do sistema (simplificado para coreografado)"""
    try:
...
def main():
    ...
    try:
        ...
        if test_response.status_code not in [200, 201]:
            print("ERRO: Servico nao esta respondendo corretamente")
            return
    except:
        print("ERRO: Nao foi possivel conectar ao servico")
        return

    print("OK Servico Coreografado esta online")
    ...
    # Salvar em arquivo
    with open("results_choreography_detailed.json", "w") as f:
        json.dump(final_results, f, indent=2)

    print(f"\n=== SISTEMA ===")
    print(f"Ping: {system_metrics['ping_ms']:.1f}ms")
    print(f"Disponibilidade: {'OK' if system_metrics['service_available'] else 'ERRO'}")

    print(f"\nResultados detalhados salvos em: results_choreography_detailed.json")
